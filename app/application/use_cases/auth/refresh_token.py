...
class RefreshTokenService:
    def __init__(self, user_repo, tenant_repo, token_service, cache_service):
    ...
    def execute(self, refresh_token: str) -> dict:
        ...
        if is_blacklisted:
            logger.critical(
                "🚨 SECURITY ALERT: Attempted reuse of a blacklisted refresh token!"
            )
        ...
        logger.info(
            f"🔄 Tokens rotated and refreshed successfully for user: {user.email}"
        )
